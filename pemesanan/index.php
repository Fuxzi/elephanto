...
SELECT 
    ...
    pm.tanggal,
    pm.status,
    GROUP_CONCAT(CONCAT(COALESCE(mm.merk, m.merk), ' ', COALESCE(mm.tipe, m.tipe), ' (', dp.jumlah, ')') SEPARATOR ', ') AS mobil,
...
LEFT JOIN mobil m ON dp.id_mobil = m.id_mobil
LEFT JOIN master_mobil mm ON m.id_master = mm.id_master
GROUP BY pm.id_pemesanan, pb.nama, pm.tanggal, pm.status
...
<a class="btn" href="tambah.php">+ Tambah Pemesanan</a> |
...
<?php if (mysqli_num_rows($data) > 0) { ?>
    ...
    <tr>
        ...
        <td>
            <a class="btn btn-secondary" href="edit.php?id=<?= $row['id_pemesanan']; ?>">Edit</a> |
            <a class="btn btn-danger" href="batal.php?id=<?= $row['id_pemesanan']; ?>" onclick="return confirm('Batalkan pemesanan ini?')">Batalkan</a>
        </td>
    </tr>
    <?php } ?>
<?php } else { ?>
    <tr>
        <td colspan="8">Belum ada data pemesanan.</td>
    </tr>
<?php } ?>
</table>
</div></div>
